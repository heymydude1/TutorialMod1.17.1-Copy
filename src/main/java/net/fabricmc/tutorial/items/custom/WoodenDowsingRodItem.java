...
import net.fabricmc.tutorial.util.ModTags;
...
public class WoodenDowsingRodItem extends Item {
    public WoodenDowsingRodItem(Settings settings) {
        super(settings);
    }
    ...
    private void addNbtToDataTablet(PlayerEntity player, BlockPos pos, Block blockBelow) {
        ItemStack dataTablet =
                player.getInventory().getStack(getFirstInventoryIndex(player));
    ...
    private boolean hasPlayerDataTablet(PlayerEntity player) {
        return hasPlayerStackInInventory(player);
    }

    private boolean hasPlayerStackInInventory(PlayerEntity player) {
        for(int i = 0; i < player.getInventory().size(); i++) {
            ItemStack currentStack = player.getInventory().getStack(i);
            if (!currentStack.isEmpty() && currentStack.isItemEqual(new ItemStack(ModItems.DATA_TABLET))) {
                return true;
            }
        }
    ...
    private int getFirstInventoryIndex(PlayerEntity player) {
        for(int i = 0; i < player.getInventory().size(); i++) {
            ItemStack currentStack = player.getInventory().getStack(i);
            if (!currentStack.isEmpty() && currentStack.isItemEqual(new ItemStack(ModItems.DATA_TABLET))) {
                return i;
